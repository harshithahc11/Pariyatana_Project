...
const BranchDropDownComponent = ({ label, name, arrayData, stateData, setStateData, options, id, issubmit, inputerror, dropdown, proId }) => {
    const setcomp = useCallback((data) => {
        if (data) {
            const stat = options.find(s => s.id === data.branch_id);
